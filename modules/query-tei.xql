        ...
        return            
            if ($target-texts) then
                ...
                return
                    ...
                    $config:data-root  || "/people" ! doc(. || "/" || $text)//tei:person[ft:query(., 'name:(' || $query || '*)')] |
                    $config:data-root  || "/people" ! doc(. || "/" || $text)//tei:person[ft:query(., 'person-note:(' || $query || '*)')]  |
                    $config:data-root  || "/places" ! doc(. || "/" || $text)//tei:place[ft:query(., 'lname:(' || $query || '*)')] 
            else
                ...
                collection($config:data-root || "/people")//tei:person[ft:query(., 'name:(' || $query || '*)', query:options($sortBy))] |
                collection($config:data-root || "/people")//tei:person[ft:query(., 'person-note:(' || $query || '*)', query:options($sortBy))] |
                collection($config:data-root || "/places")//tei:listPlace/tei:place[ft:query(., 'lname:(' || $query || '*)', query:options($sortBy))] 
    else ()
};

declare function teis:query-metadata($field as xs:string, $query as xs:string, $sort as xs:string) {
    for $doc in collection($config:data-root || "/briefe")//tei:text[ft:query(., $field || ":(" || $query || ")", query:options($sort))]
    return
...
declare function teis:autocomplete($doc as xs:string?, $fields as xs:string+, $q as xs:string) {
    ...
    return
        switch ($field)
            case "author" return
                collection($config:data-root|| "/briefe")/ft:index-keys-for-field("author", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "file" return
                collection($config:data-root || "/briefe")/ft:index-keys-for-field("file", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "keyword" return
                collection($config:data-root || "/briefe")/ft:index-keys-for-field("keyword", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "correspondent" return
                collection($config:data-root || "/briefe")/ft:index-keys-for-field("correspondent", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "text" return
...
declare function teis:get-breadcrumbs($config as map(*), $hit as node(), $parent-id as xs:string) {
    let $uri := if (starts-with($parent-id, 'B')) then
        'briefe/'
    else
        ()
    let $work := root($hit)/*
    ...
    return
        <div class="breadcrumbs">
            <a class="breadcrumb" href="{$uri}{$parent-id}">{$work-title}</a>
            {
                ...
                return
                    <a class="breadcrumb" href="{$uri}{$parent-id || "?action=search&amp;root=" || $id || "&amp;view=" || $config?view || "&amp;odd=" || $config?odd}">
